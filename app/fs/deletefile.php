...
if ($existing["owner"] != $session["id"] && !$session["admin"]) {
    error_message("Permission denied", "You are not the owner of this " . $type . ". Only the owner can delete it.", $continue, "Okay");
    die;
}
...
if ($existing["owner"] != $session["id"] && $session["admin"]) {
    $error_msg .= <<<HTML
    <br><br>
    <font color="#ff0000"><b>WARNING, ADMIN ACTION:</b></font><br>You DO NOT own this $type. You should only perform these kinds of actions if it's for moderation.
    HTML;
}
